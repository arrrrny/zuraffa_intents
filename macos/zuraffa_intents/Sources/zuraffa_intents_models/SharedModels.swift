...
open class SharedMedia: Codable {
    ...
    public init(attachments: [SharedAttachment]?, conversationIdentifier: String?, content: String?, speakableGroupName: String?, serviceName: String?, senderIdentifier: String?, imageFilePath: String?, subject: String?, recipientIdentifiers: [String?]?) {
    ...
    public class func fromMap(map: Dictionary<String, Any?>?) -> SharedMedia? {
        if let _map = map {
            return SharedMedia.init(
                attachments: (_map["attachments"] as? Array<Dictionary<String,Any>>)?.compactMap{ SharedAttachment.fromMap(map: $0)},
                conversationIdentifier: _map["conversationIdentifier"] as? String,
                content: _map["content"] as? String,
    ...
    public class func fromJson(data: Data?) -> SharedMedia? {
        if let _json = data {
            ...
            if let _map = map {
                return SharedMedia(
                    attachments: (_map["attachments"] as? Array<Dictionary<String,Any>>)?.map{ SharedAttachment(path: $0["path"] as! String, type: SharedAttachmentType(rawValue: $0["type"] as? Int ?? SharedAttachmentType.file.rawValue) ?? SharedAttachmentType.file )},
                    conversationIdentifier: _map["conversationIdentifier"] as? String,
                    content: _map["content"] as? String,
                    speakableGroupName: _map["speakableGroupName"] as? String,
                    serviceName: _map["serviceName"] as? String,
