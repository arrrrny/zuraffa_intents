...
import 'package:zuraffa_intents/tdd/002-publishable-plugin/u6_subject.dart'
    as subject;

void main() {
  group('U6 (FR-006, MethodChannelShareIntentPort.recordSentMessage)', () {
    test(
      'U6 — `recordSentMessage` MUST send the single-element envelope',
      () async {
        ...
        expect(
          envelope.length,
          1,
          reason: 'the pigeon native handler casts argument 0',
        );
        ...
        await expectLater(
          ...
          throwsA(
            isA<PlatformException>().having((e) => e.code, 'code', 'SEND_ERR'),
          ),
        );
      },
    );
