...
import 'package:zuraffa_intents/tdd/002-publishable-plugin/u1_subject.dart'
    as subject;

void main() {
  group('U1 (FR-001, publishability)', () {
    test(
      'U1 — The package MUST be publish-ready for pub.dev: `pubspec.yaml`',
      () {
        ...
        final block = RegExp('description: >-$newline((?:  .*$newline)+)')
            .firstMatch(pubspec)!
            .group(1)!;
        ...
        // The fleet-standard MIT license text is present.
        expect(files['license']!, contains('MIT License'));
        expect(files['license']!, contains('Ahmet TOK'));
        ...
        expect(
          files['changelog']!.split(newline).take(6).join(newline),
          contains(version),
        );
      },
    );
