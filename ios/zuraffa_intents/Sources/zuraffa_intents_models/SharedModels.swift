...
open class SharedAttachment: Codable {
    ...
    public class func fromMap(map: [String: Any?]?) -> SharedAttachment? {
        if let _map = map {
            return SharedAttachment.init(
                path: _map["path"] as! String,
                type: SharedAttachmentType.init(rawValue: _map["type"] as! Int)!)
        } else {
            return nil
        }
    }
...
open class SharedMedia: Codable {
    ...
    public class func fromMap(map: [String: Any?]?) -> SharedMedia? {
        if let _map = map {
            return SharedMedia.init(
                attachments: (_map["attachments"] as? [[String: Any]])?.compactMap {
                    SharedAttachment.fromMap(map: $0)
    ...
    public class func fromJson(data: Data?) -> SharedMedia? {
        if let _json = data {
            let map = try? JSONSerialization.jsonObject(with: _json) as? [String: Any]
            if let _map = map {
                return SharedMedia(
                    attachments: (_map["attachments"] as? [[String: Any]])?.map {
                        SharedAttachment(
                            path: $0["path"] as! String,
                            type: SharedAttachmentType(
                                rawValue: $0["type"] as! Int? ?? SharedAttachmentType.file.rawValue)
